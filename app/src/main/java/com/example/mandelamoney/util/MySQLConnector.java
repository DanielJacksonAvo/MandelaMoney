...
public class MySQLConnector {
    ...
    public static Student updateStudentDetails(
        ...
        // The SQL call now has 6 parameters (5 IN, 1 OUT) instead of 7
        try (CallableStatement callableStatement = currentConnection.prepareCall("{CALL UpdateStudentDetails(?, ?, ?, ?, ?, ?)}")) {
            // Set the input parameters
            callableStatement.setString(1, userEmail);
            callableStatement.setString(2, userPassword);
            callableStatement.setString(3, newFirstName);    // Index is now 3
            callableStatement.setString(4, newLastName);      // Index is now 4
            callableStatement.setString(5, newStudentNumber); // Index is now 5

            // Register the output parameter
            callableStatement.registerOutParameter(6, Types.INTEGER); // Index is now 6
            ...
            int result = callableStatement.getInt(6); // Index is now 6
            if (result == 1) {
                Log.d("MySQLConnector", "Student details updated successfully.");
                // Create the updated Student object using the original email
                updatedStudent = new Student(userEmail); // Use original userEmail
                ...
                Log.e("MySQLConnector", "Failed to update student details. Check credentials or database state.");
            }
        } catch (SQLException e) {
